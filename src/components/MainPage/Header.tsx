import React from "react";
import {
  ThemeDivForOneStyle,
  ThemeHeader,
  ThemeRightSide,
  ThemeToogleMain,
} from "./MainPage.style";
import { AppInput } from "../UI/AppInput/AppInput";
import { AppLink } from "../UI/AppLink/AppLink";
import { MenuFavorites } from "./MenuofLoves";

import Button from "react-bootstrap/Button";
import ButtonGroup from "react-bootstrap/ButtonGroup";
import Form from "react-bootstrap/Form";
import { useTheme } from "../hooks/use-theme";

import "../../../src/App.css";

export const Header = () => {
  const { theme, setTheme } = useTheme();

  const handleLightThemeClick = () => {
    setTheme("light");
  };
  const handleDarkThemeClick = () => {
    setTheme("dark");
  };
  return (
    <ThemeHeader className="boxBackExchange">
      <div className="logo">
        <h1 className="mainTitles">Michtel</h1>
        <svg
          height="60px"
          width="60px"
          className="iconTheme"
          version="1.1"
          id="Layer_1"
          xmlns="http://www.w3.org/2000/svg"
          xmlnsXlink="http://www.w3.org/1999/xlink"
          viewBox="0 0 295.238 295.238"
          xmlSpace="preserve"
        >
          <g>
            <g>
              <polygon
                fill="#F9BA48"
                points="144.771,185.543 81.638,200 125.724,152.552 128.571,171.429 		"
              />
              <polygon
                fill="#F9BA48"
                points="150.467,138.267 213.6,123.81 169.514,171.257 166.667,152.381 		"
              />
              <path
                className="iconTheme"
                d="M163.486,29.548c2-3.014,3.181-6.619,3.181-10.5C166.667,8.543,158.124,0,147.619,0
			s-19.048,8.543-19.048,19.048c0,3.881,1.181,7.486,3.181,10.5C65.686,37.424,14.286,93.757,14.286,161.905
			c0,73.519,59.814,133.333,133.333,133.333s133.333-59.814,133.333-133.333C280.952,93.757,229.552,37.424,163.486,29.548z
			 M138.095,19.048c0-5.252,4.271-9.524,9.524-9.524s9.524,4.271,9.524,9.524s-4.271,9.524-9.524,9.524S138.095,24.3,138.095,19.048
			z M147.619,285.714c-68.271,0-123.809-55.538-123.809-123.81s55.538-123.81,123.809-123.81s123.809,55.538,123.809,123.81
			S215.89,285.714,147.619,285.714z"
              />
              <path
                className="iconTheme"
                d="M147.619,52.381c-60.395,0-109.524,49.129-109.524,109.524s49.129,109.524,109.524,109.524
			S257.143,222.3,257.143,161.905S208.014,52.381,147.619,52.381z M147.619,261.905c-55.138,0-100-44.862-100-100
			s44.862-100,100-100s100,44.862,100,100S202.757,261.905,147.619,261.905z"
              />
              <path
                className="iconTheme"
                d="M147.619,133.333c-15.757,0-28.571,12.814-28.571,28.571s12.814,28.571,28.571,28.571
			s28.571-12.814,28.571-28.571S163.376,133.333,147.619,133.333z M147.619,180.952c-10.505,0-19.048-8.543-19.048-19.048
			s8.543-19.048,19.048-19.048s19.048,8.543,19.048,19.048C166.667,172.41,158.124,180.952,147.619,180.952z"
              />
              <polygon
                className="iconTheme"
                points="138.095,84.776 154.595,109.524 166.667,109.524 166.667,71.429 157.143,71.429 
			157.143,96.176 140.643,71.429 128.571,71.429 128.571,109.524 138.095,109.524 		"
              />
              <polygon
                className="iconTheme"
                points="238.095,152.381 238.095,142.857 204.762,142.857 204.762,180.952 238.095,180.952 
			238.095,171.429 214.286,171.429 214.286,166.667 238.095,166.667 238.095,157.143 214.286,157.143 214.286,152.381 		"
              />
              <path
                className="iconTheme"
                d="M154.762,214.286h-14.286c-6.567,0-11.905,5.338-11.905,11.905s5.338,11.905,11.905,11.905h14.286
			c1.31,0,2.381,1.071,2.381,2.381s-1.071,2.381-2.381,2.381h-14.286c-1.31,0-2.381-1.071-2.381-2.381v-2.381h-9.524v2.381
			c0,6.567,5.338,11.905,11.905,11.905h14.286c6.567,0,11.905-5.338,11.905-11.905s-5.338-11.905-11.905-11.905h-14.286
			c-1.31,0-2.381-1.071-2.381-2.381s1.071-2.381,2.381-2.381h14.286c1.31,0,2.381,1.071,2.381,2.381v2.381h9.524v-2.381
			C166.667,219.624,161.329,214.286,154.762,214.286z"
              />
              <polygon
                className="iconTheme"
                points="83.157,160.433 80.224,142.857 67.395,142.857 64.462,160.433 61.857,142.181 
			52.429,143.533 57.776,180.952 70.7,180.952 73.81,162.3 76.919,180.952 89.843,180.952 95.19,143.533 85.762,142.181 		"
              />
            </g>
          </g>
        </svg>
      </div>
      <AppInput
        type="search"
        id="search"
        name="search"
        inputPlaceholder="Поиск"
      />

      <ThemeRightSide>
        <div className="app__container w-100 h-100 d-flex flex-column">
          <div className="p-3 d-flex justify-content-end">
            <ButtonGroup aria-label="Theme toggle">
              <Button variant="secondary" onClick={handleLightThemeClick}>
                Light
              </Button>
              <Button variant="secondary" onClick={handleDarkThemeClick}>
                Dark
              </Button>
            </ButtonGroup>
          </div>
          {/* <div className="flex-grow-1 d-flex flex-column justify-content-center align-items-center">
            <FaCoffee size={100} className="app__logo mb-5" />
            <div className="d-flex">
              <Form.Control
                className="app__input mr-3"
                type="email"
                placeholder="Enter email"
              />
              <Button className="app__button" type="button">
                Subscribe
              </Button>
            </div>
          </div> */}
        </div>
        {/* <MenuFavorites /> */}
      </ThemeRightSide>
    </ThemeHeader>
  );
};
